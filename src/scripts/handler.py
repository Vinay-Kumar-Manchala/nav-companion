...
class LogicHandler():
    ...
    def get_list_of_mutual_funds(self):
        funds_query = "SELECT * FROM mutual_funds ORDER BY fund_type;"
        ...
        for each_data in funds_data:
            try:
                row_template = "{ mutualFund: 'val1', fundType: 'val2', oneTimeAvailable: 'val3', greenStocks: 'val4', " \
                               "redStocks: 'val5', greenAvg: 'val6', redAvg: 'val7' }".\
                    replace("val1", each_data["mf_name"]).replace("val2", each_data["fund_type"]).\
                    replace("val3", str(each_data["onetime_available"])).replace("val4", "-" if each_data["green_stocks"] in ["", None] else str(each_data["green_stocks"])).\
                    replace("val5", "-" if each_data["red_stocks"] in ["", None] else str(each_data["red_stocks"])).replace("val6", "-" if each_data["green_avg"] in ["", None] else str(each_data["green_avg"])).\
                    replace("val7", "-" if each_data["red_avg"] in ["", None] else str(each_data["red_avg"]))
                r_temps.append(row_template)
            except Exception as e:
                print(str(e))
                continue

        template = template.replace('__data__', "[" + ", ".join(r_temps) + "]")
        return template
